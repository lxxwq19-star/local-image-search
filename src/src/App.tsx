...
interface IndexProgress {
  ...
  status: string;
  current_file?: string;
  remaining?: number;   // Phase 2: remaining images to encode
  encoded?: number;      // Phase 2: already encoded count
}
...
function App() {
  ...
  const handleSearch = async (query: string, searchMode: 'text' | 'image') => {
    ...
    try {
      ...
      if (searchMode === 'text') {
        ...
        results = await invoke<{ images: ImageResult[] }>('search_by_image', {
          imagePath: query,
          topK: 9999,
        });
      }
  ...
  const handleDeletePath = async (path: string) => {
    if (!confirm(`确定要删除 "${path}" 及其索引数据吗？`)) return;
    try {
  ...
  const handleRebuildAll = async () => {
    if (!confirm('确定要重建所有已启用路径的索引吗？')) return;
    try {
      setIndexing(true);
      setIndexProgress({ counted: 0, indexed: 0, status: 'scanning' });
          ...
          {searchHistory.length > 0 ? (
            <div className="history-tags">
              {searchHistory.map((q, i) => (
                <button key={i} className="history-tag" onClick={() => searchFromHistory(q)}>
                  {q}
                </button>
              ))}
            </div>
          ...
          <div className="paths-section">
            ...
            <div className="paths-list">
              {indexPaths.length === 0 ? (
                ...
                indexPaths.map((p) => (
                  <div key={p.id} className={`path-item ${!p.enabled ? 'path-item-disabled' : ''}`}>
                    {/* 启用/禁用开关 */}
                    <button
                    ...
                    >
                      <span className={`toggle-dot ${p.enabled ? 'on' : ''}`}>
                        {p.enabled ? '✓' : ''}
                      </span>
                    </button>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ fontSize: 12, color: '#999', marginTop: 6 }}>
                {indexProgress.status === 'scanning'
                  ...
                  ? `向量编码中 ${(indexProgress as any).encoded?.toLocaleString() || 0} / ${indexProgress.counted.toLocaleString()}，剩余 ${(indexProgress as any).remaining?.toLocaleString() || '?'} 个`
